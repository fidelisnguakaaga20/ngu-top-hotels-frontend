...
import { useFetchBlogByIdQuery, usePostBlogsMutation, useUpdateBlogMutation } from '../../../redux/features/blogs/blogsApi';
    ...
    <div className="bg-white md:p-8 p-2">
      ...
      >
        <div className="space-y-4">
          <label className="font-semibold text text-xl">Blog Title:</label>
          <input type="text" 
          defaultValue={blog?.post?.title}
          onChange={(e) => setTitle(e.target.value)}
          className="w-full inline-block bg-bgPrimary focus:outline-none px-5 py-3"
          placeholder="Ex: Marina del Ray Mariott..." required/>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          ...
          className="w-full inline-block bg-bgPrimary focus:outline-none px-5 py-3"
          placeholder="https://x.com/nguakaaga..." required
          />
        </div>
        ...
        <button type="submit"
        disabled={isLoading}
        className="w-full mt-5 bg-primary hover:bg-indigo-500 text-white font-medium py-3 rounded-md">Update Blog</button>
      </form>
    </div>
  )
}
